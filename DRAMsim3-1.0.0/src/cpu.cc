...
void RandomCPU::ClockTick() {
    ...
    if (get_next_) {
        last_addr_ = gen();
        last_write_ = (gen() % 3 == 0);
        // Print out the address and the type of transaction
        // std::cout << "Last Addr: 0x" << std::hex << last_addr_ << " Last write: " << last_write_ << std::endl;

    }
    get_next_ = memory_system_.WillAcceptTransaction(last_addr_, last_write_);
    if (get_next_) {
        string tx_type = last_write_ ? "Write" : "Read";
        std::cout << "Cycle: " << std::dec << clk_ << " " << tx_type << " Addr: 0x" << std::hex << last_addr_ << std::endl;
        memory_system_.AddTransaction(last_addr_, last_write_);
    } else {
        std::cout << "Cycle: " << std::dec << clk_ << " Not accepted" << std::endl;
    }
    clk_++;
    return;
}
