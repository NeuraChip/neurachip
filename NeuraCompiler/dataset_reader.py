...
def dram_raw_data_writer(mat_A, mat_B, mat_C):
	...
	with open(dram_file_name, 'w+') as f:
		...
		# Close the file
		f.close()
		# Copy dram_raw_data.csv to dram_raw_data_0.csv and dram_raw_data_1.csv
		shutil.copy(dram_file_name, dram_file_path + "dram_raw_data_0.csv")
		shutil.copy(dram_file_name, dram_file_path + "dram_raw_data_1.csv")
...
# Create a dense matrix of size x_dim * y_dim
def create_dense_matrix(x_dim, y_dim):
	dense_matrix = np.ones((x_dim, y_dim))
	# CSR format
	csr_dense_matrix = csr_matrix(dense_matrix)
	# CSC format
	csc_dense_matrix = csc_matrix(dense_matrix)
	return Matrix(name='dense_matrix', type='CSR_CSC', row_ptr=csr_dense_matrix.indptr, col_indices=csr_dense_matrix.indices, data=csr_dense_matrix.data, col_ptr=csc_dense_matrix.indptr, row_indices=csc_dense_matrix.indices, x_dim=x_dim, y_dim=y_dim)

def read_matrix_market_file(MTX_DATASET_NAME):
	MTX_DATASET_ARR = ['2cubes_sphere', 'CollegeMsg', 'filter3D', 'mario002']
	# Ensure that the dataset name is valid
	if MTX_DATASET_NAME not in MTX_DATASET_ARR:
		raise ValueError("Invalid dataset name")
	mtx_matrix = mmread('snap/' + MTX_DATASET_NAME + '/' + MTX_DATASET_NAME + '.mtx')
	csr_mtx_matrix = mtx_matrix.tocsr()
	csc_mtx_matrix = mtx_matrix.tocsc()
	matrix_obj = Matrix(name=MTX_DATASET_ARR[0], type='CSR_CSC', row_ptr=csr_mtx_matrix.indptr, col_indices=csr_mtx_matrix.indices, data=csr_mtx_matrix.data, col_ptr=csc_mtx_matrix.indptr, row_indices=csc_mtx_matrix.indices, x_dim=mtx_matrix.shape[0], y_dim=mtx_matrix.shape[1])
	return matrix_obj
